...
class Title {
  ...
  constructor({ gl, plane, renderer, text, textColor = '#545050', font = '30px Caveat' }: TitleProps) {
    autoBind(this);
    this.gl = gl;
    this.plane = plane;
...
class Media {
  ...
  onResize({ screen, viewport }: { screen?: ScreenSize; viewport?: Viewport } = {}) {
    ...
    // Responsive sizing - smaller cards on mobile and laptop
    const isMobile = this.screen.width < 768;
    const cardHeightBase = isMobile ? 500 : 700;
    const cardWidthBase = isMobile ? 400 : 550;

    this.plane.scale.y = (this.viewport.height * (cardHeightBase * this.scale)) / this.screen.height;
    this.plane.scale.x = (this.viewport.width * (cardWidthBase * this.scale)) / this.screen.width;
    this.plane.program.uniforms.uPlaneSizes.value = [this.plane.scale.x, this.plane.scale.y];
    this.padding = isMobile ? 1 : 1.5;
...
class App {
  ...
  addEventListeners() {
    ...
    canvas.addEventListener('mousewheel', this.boundOnWheel, { passive: false });
...
interface CircularGalleryProps {
  items?: { image: string; text: string; path?: string }[];
  bend?: number;
  textColor?: string;
  borderRadius?: number;
  font?: string;
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    // Add click handler to the canvas if onItemClick is provided
    if (onItemClick && app.renderer && app.renderer.gl && app.renderer.gl.canvas) {
      const canvas = app.renderer.gl.canvas as HTMLCanvasElement;
      ...
      let startY = 0;
      let startTime = 0;
      const dragThreshold = 10; // pixels - increased for easier mobile taps
      const clickTimeThreshold = 300; // ms
